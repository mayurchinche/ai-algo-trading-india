...
Replace current device execution/UI adapter with one dedicated shared backend account, no sign-in form. Backend computes observations; database lease, revision checks and idempotent requests prevent duplicate cross-device activity. Preserve previous device records for export/reconciliation. Code and tests implemented; database migration, Vercel activation, historical reconciliation and new APK release are pending deployment access. See docs/SHARED-USER1-PAPER.md.
