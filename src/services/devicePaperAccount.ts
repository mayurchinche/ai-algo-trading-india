...
import {newPaperAccount,advancePaper,PAPER_VERSION} from '../../server/paperEngine.js';
...
export function devicePaperSymbols():string[]{return read().state.orders.filter((o:RecordData)=>!['CLOSED','CANCELLED'].includes(o.status)).map((o:RecordData)=>o.symbol);}
export async function devicePaperRequest(action?:RecordData,after=0,until?:number,orderId?:string){return locked(()=>{
 ...
 if(action){let result;
  if(action.transfer)result=transferPaperFunds(ledger.state,action.transfer);
 ...
 if(changed){ledger.revision++;save(ledger);}
 const filtered=ledger.events.filter(e=>e.id>after&&e.id<=(until??ledger.state.sequence)&&(!orderId||e.orderId===orderId));
...
export async function runDevicePaper(stocks:DiscoveredStock[],quotes:Map<string,MarketQuote>,marketOpen:boolean,now=Date.now()){
 return locked(()=>{
  ...
  const ledger=read();const candidates=stocks.filter(s=>!strongSignalRejection(s,now)).map(s=>({symbol:s.symbol,signalId:s.signalId||strongSignalId(s,now),signalTime:s.firstSignalAt||s.generatedAt,side:s.overallScore>0?'BUY':'SELL',score:s.overallScore,stop:s.foAnalysis.suggestedStopLoss,target:s.foAnalysis.suggestedTarget}));
  ...
  const result=advancePaper(ledger.state,{now,quotes:[...combined].map(([symbol,q])=>({symbol,...q})),candidates,marketOpen,acceptEntries:ledger.enabled});
