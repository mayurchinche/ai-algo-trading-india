...
export function PaperFundsPanel({balance,state,disabled,onAction}:{balance:PaperBalance|null;state?:FundingState;disabled:boolean;onAction:(action:Record<string,unknown>)=>Promise<boolean>}) {
 ...
 return <div className="space-y-4"><div className="metric-grid">{[
 ['Net paper capital',balance?.principal],['All-time net trade P&L',balance?.realized],['Ledger balance',balance?.balance],['Reserved for orders',balance?.reserved],['Open position P&L estimate',balance?.positionPnl],['Estimated account equity',balance?.equity],['Available to withdraw',balance?.available]
 ...
 {!state?.legacyImport&&<section className="card space-y-2"><h3>Reconcile earlier device history</h3><p>Before starting new trades or adding funds, pause entries and import on the device containing your previous journal. Existing trades, open positions and archived P&amp;L will be carried forward once. Missing records are not invented.</p><button disabled={disabled} onClick={()=>void importHistory()}>Import earlier paper journal</button></section>}
