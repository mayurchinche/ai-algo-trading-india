...
export function PaperFundsPanel({balance,state,disabled,onAction}:{balance:PaperBalance|null;state?:FundingState;disabled:boolean;onAction:(action:Record<string,unknown>)=>Promise<boolean>}) {
 ...
 {!state?.legacyImport&&<section className="card space-y-2"><h3>Earlier device history</h3><p>Earlier device records are preserved separately. Export them for reconciliation before importing into user1; they are not automatically combined with the shared balance.</p><button onClick={exportHistory}>Export earlier device account</button></section>}
