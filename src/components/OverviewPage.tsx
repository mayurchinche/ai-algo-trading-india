...
export function OverviewPage({ nifty: _nifty }: { nifty: import('../services/liveData').LiveStock | null }) {
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wide">Cumulative P&L (26 Trading Days)</h3>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={dailyPnl}>
              ...
              <XAxis dataKey="date" tick={{ fontSize: 9, fill: '#64748B' }} axisLine={false} tickLine={false} tickFormatter={(v) => v.slice(5)} />
              <YAxis tick={{ fontSize: 9, fill: '#64748B' }} axisLine={false} tickLine={false} tickFormatter={(v) => `${(v/1000).toFixed(0)}K`} />
              <Tooltip contentStyle={{ background: '#FFFFFF', border: '1px solid #E2E8F0', color: '#1A202C', borderRadius: 8, fontSize: 11 }} formatter={(v) => [`₹${Number(v).toLocaleString()}`, '']} />
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wide">Daily P&L</h3>
            <span className="text-[10px] text-[var(--text-secondary)]">{portfolio.profitableDays}/{portfolio.tradingDays} profitable days</span>
          </div>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={dailyPnl}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,0.06)" />
              <XAxis dataKey="date" tick={{ fontSize: 9, fill: '#64748B' }} axisLine={false} tickLine={false} tickFormatter={(v) => v.slice(8)} />
              <YAxis tick={{ fontSize: 9, fill: '#64748B' }} axisLine={false} tickLine={false} tickFormatter={(v) => `${(v/1000).toFixed(0)}K`} />
              <Tooltip contentStyle={{ background: '#FFFFFF', border: '1px solid #E2E8F0', color: '#1A202C', borderRadius: 8, fontSize: 11 }} formatter={(v) => [`₹${Number(v).toLocaleString()}`, '']} />
            ...
            {strategyStats.map((s) => (
              <div key={s.name} className="p-3 rounded-lg bg-[var(--bg-alt)] border border-[var(--border)]">
                ...
                <div className="grid grid-cols-3 gap-2 text-[10px] text-[var(--text-secondary)]">
                  <div>WR: <span className="text-[var(--text)] font-medium">{s.winRate}%</span></div>
      ...
      {/* Portfolio details */}
      <div className="card">
        <h3 className="text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wide mb-3">Portfolio Details</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 text-xs">
          <div><span className="text-[var(--text-secondary)]">Invested</span><div className="font-medium">₹{(portfolio.investedValue / 1000).toFixed(0)}K</div></div>
          <div><span className="text-[var(--text-secondary)]">Available Margin</span><div className="font-medium">₹{(portfolio.availableMargin / 100000).toFixed(2)}L</div></div>
          <div><span className="text-[var(--text-secondary)]">Realized P&L</span><div className="font-medium text-[var(--green)]">+₹{portfolio.realizedPnl.toLocaleString()}</div></div>
          <div><span className="text-[var(--text-secondary)]">Unrealized P&L</span><div className="font-medium text-[var(--green)]">+₹{portfolio.unrealizedPnl.toLocaleString()}</div></div>
          <div><span className="text-[var(--text-secondary)]">Avg Win / Loss</span><div className="font-medium">₹{portfolio.avgWin} / ₹{Math.abs(portfolio.avgLoss)}</div></div>
          <div><span className="text-[var(--text-secondary)]">Best / Worst</span><div className="font-medium"><span className="text-[var(--green)]">+₹{portfolio.largestWin.toLocaleString()}</span> / <span className="text-[var(--red)]">-₹{Math.abs(portfolio.largestLoss).toLocaleString()}</span></div></div>
          <div><span className="text-[var(--text-secondary)]">Consec. Wins</span><div className="font-medium">{portfolio.consecutiveWins}</div></div>
          <div><span className="text-[var(--text-secondary)]">Consec. Losses</span><div className="font-medium text-[var(--red)]">{portfolio.consecutiveLosses}</div></div>
          <div><span className="text-[var(--text-secondary)]">Open Positions</span><div className="font-medium">{portfolio.openPositions}</div></div>
          <div><span className="text-[var(--text-secondary)]">Avg Trades/Day</span><div className="font-medium">{portfolio.avgTradesPerDay}</div></div>
          <div><span className="text-[var(--text-secondary)]">Profitable Days</span><div className="font-medium">{portfolio.profitableDays}/{portfolio.tradingDays} ({((portfolio.profitableDays/portfolio.tradingDays)*100).toFixed(0)}%)</div></div>
          <div><span className="text-[var(--text-secondary)]">Trading Since</span><div className="font-medium">14 Jul 2026</div></div>
        </div>
      </div>
    </div>
  );
