...
export function OverviewPage({onOpenWorkspace}:{onOpenWorkspace:(signalId?:string)=>void}) {
 ...
 return <section className="trading-home space-y-6"><div className="page-title"><div><p className="eyebrow">YOUR PAPER TRADING DAY · NSE EQUITIES</p><h2>Your trading day</h2><p>Review opportunities. Follow positions. Learn from every exit.</p></div><button className="primary-button" onClick={()=>onOpenWorkspace()}>Paper trades &amp; funds →</button></div>
 ...
 <p className="notice">Paper money only · Saved on this device · Monitoring runs while the app is open and visible. Public quote snapshots and estimated costs cannot reproduce every exchange fill.</p>
 </section>;
}
