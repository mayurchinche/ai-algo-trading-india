...
export function advancePaper(previous, {now,quotes=[],candidates=[],marketOpen=false,acceptEntries=false}) {
 ...
 for(const o of state.orders) {
  ...
  if(o.filled<o.quantity && (now-Date.parse(o.submittedAt)>120000 || !acceptEntries || day(o.submittedAt)!==date || minute(now)>=915)) {
  ...
  if(Date.parse(q.timestamp)<Date.parse(o.submittedAt)) continue;
  ...
  if(o.filled<o.quantity && minute(now)<915 && !overnight) {
   ...
   event(o,'ENTRY_FILL',{...f,quoteTime:q.timestamp,source:q.source});
  }
 }
 ...
 for(const s of candidates) {
  ...
  if(!acceptEntries||!marketOpen||minute(now)<555||minute(now)>=915||!valid(q)||today.length>=3||dailyRealized+openPnl<=-Math.max(0,fundedCapital(state)+state.realized-dailyRealized)*.02||occupied.some(o=>day(o.submittedAt)!==date || o.filled>o.exited&&!bySymbol.has(o.symbol))) continue;
  if(!s.signalId||!['BUY','SELL'].includes(s.side)||!Number.isFinite(s.score)||Math.abs(s.score)<70||Math.abs(s.score)>100||!Number.isFinite(Date.parse(s.signalTime))||now-Date.parse(s.signalTime)<0||now-Date.parse(s.signalTime)>120000) continue;
  if(![s.stop,s.target].every(n=>Number.isFinite(n)&&n>0) || !(s.side==='BUY'?s.stop<q.price&&q.price<s.target:s.target<q.price&&q.price<s.stop)) continue;
  ...
  const o={id:s.signalId,signalId:s.signalId,symbol:s.symbol,side:s.side,score:s.score,signalTime:s.signalTime,submittedAt:at,referencePrice:reference,riskBudget:equity*.005,stop:s.stop,target:s.target,quantity,filled:0,exited:0,entryValue:0,exitValue:0,status:'PENDING',lastPrice:q.price,lastQuoteTime:q.timestamp,monitoringGap:false};
  state.orders.push(o);today.push(o);reserved+=quantity*reference+40;event(o,'ORDER_SUBMITTED',{quantity,signalTime:s.signalTime,quoteTime:q.timestamp});
 }
